...
from utils.tools import EarlyStopping, adjust_learning_rate, visual, warm_up
    ...
    def train(self, setting):
        ...
        if self.args.warmup_epochs > 0:
            model_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer=model_optim,
                                                                         T_max=self.args.train_epochs - self.args.warmup_epochs)
            print('We will warm up', self.args.warmup_epochs, 'epochs!')
        else:
            model_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer=model_optim,
                                                                         T_max=self.args.train_epochs)
            print('We do not use warm up')
        ...
        for epoch in range(self.args.train_epochs):
            ...
            if self.args.warmup_epochs > 0 and epoch >= self.args.warmup_epochs:
                ...
                print(f'Updating learning rate to {model_scheduler.get_last_lr()[0]:.7f}')
