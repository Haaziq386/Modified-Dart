...
# Script to compare HtulTS vs SimMTM vs TimeDART for various d_model values on ETTh2 dataset
# Usage: bash scripts/compare_dmodel_etth2.sh
...
echo "d_model Comparison: HtulTS vs SimMTM vs TimeDART on ETTh2" > "$SUMMARY_FILE"
...
run_htults_pretrain() {
    ...
    python -u run.py \
        ...
        --data_path ETTh2.csv \
        --model_id ETTh2_len${input_len}_dm${d_model} \
        --model HtulTS \
        --data ETTh2 \
        --features M \
        --input_len $input_len \
