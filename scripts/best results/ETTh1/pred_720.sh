...
# Best HTULTS config from latest summary: outputs/test_results/htults_grid_ETTh1_20260419_064552/summary.txt
# Dataset=ETTh1, pred_len=720
...
python -u run.py \
  ...
  --use_cpc "1" \
  --cpc_lambda "0.2" \
  --cpc_freq_mask_ratio "0.5" \
  --cpc_time_mask_ratio "0.001" \
  --cpc_use_learned_mask "0" \
  --cpc_loss_type "smooth_l1" \
  --use_forgetting "1" \
