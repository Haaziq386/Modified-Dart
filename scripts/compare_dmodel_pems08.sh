...
# Script to compare HtulTS vs SimMTM vs TimeDART for various d_model values on PEMS08 dataset
# Usage: bash scripts/compare_dmodel_PEMS08.sh

set -o pipefail  # Exit if any command in a pipe fails, but allow individual commands to fail

DATASET="PEMS08"
TIMESTAMP=$(date +"%Y%m%d_%H%M%S")
LOG_DIR="./outputs/logs/dmodel_comparison_PEMS08_${TIMESTAMP}"
RESULTS_DIR="./outputs/test_results/dmodel_comparison_PEMS08_${TIMESTAMP}"
...
echo "d_model Comparison: HtulTS vs SimMTM vs TimeDART on PEMS08" > "$SUMMARY_FILE"
...
run_htults_pretrain() {
    ...
    for use_forgetting in 0 1; do
        ...
        python -u run.py \
            ...
            --model_id PEMS08_len${input_len}_dm${d_model}_${run_tag} \
            --model HtulTS \
            --data PEMS08 \
            --features M \
            --input_len $input_len \
...
run_simmtm_pretrain() {
    ...
    python -u run.py \
        ...
        --root_path ./datasets/PEMS/ \
        --data_path PEMS08.npz \
        --model_id PEMS08_len${input_len}_dm${d_model} \
        --model SimMTM \
        --data PEMS08 \
        --features M \
        --input_len $input_len \
